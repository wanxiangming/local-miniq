...
/**
 * 它维护一个transaction池，当一个transaction被add到了一个Daylog中去，这个transaction就从池中移除，
 * 它从网络上获取到的这些transaction并不一定有Daylog可以去（因为适合这个transaction的Daylog可能还不存在），它们会暂时待在池中，直到被某个Daylog收走
 * 
 ...
 * 
 * DayLogManager()
 * 		whatINeed(CALL_BACK(timeAry))	//当它请求一些数据时，你可以做一些事，返回Deferred对象，以及transaction数据结构数组
 * 															我们降低一点它的请求速率，不要请求的太快
 * 	    nextMonth(CALL_BACK(time))
 * 	    lastMonth(CALL_BACK(time))
 * 	    
 * 		onCreate(CALL_BACK(tableId,content,time))		//当transaction被创建的时候，你可以做一些事情，返回Deferred对象，以及transactionId
 ...
 * 		filtByTableIdAry(tableIdAry)	//只显示特定日程的transaction
 * 		filtByTableAnotherNameAry(tableAnotherNameAry)	//只显示特定日程的transaction
 * 		clearFilt()						//移除filt限制
 */
 ...
 * 它控制transaction的显示
 * 如果发现有时间一样的几个transaction，它会用TimeSameTransaction处理之，这个处理同样是依transaction的显示情况做判断的
 * 它持有当天的所有transaction，如何组织是它自己的事
 * 
 * Daylog(DayFlag)
 ...
 * 		filtByTableIdAry(tableIdAry)	//只显示指定日程的transaction
 * 		filtByTableAnotherNameAry(tableAnotherNameAry)	//只显示指定日程的transaction
 * 		clearFilt()						//移除filt限制
 * 		
 * 		onCreate(CALL_BACK(tableId,content,time))		//当用户创建transaction的时候，你可以做一些事情，返回Deferred对象，包含transacitonId信息
...
var CreateTransactionModal={
	creatNew:function(){
		...
		CreateTransactionModal.initBeforShow=function(BEGINNING_TIME_OF_TODAY,SECCESS_CALL_BACK){
			var mDate=MDate.creatNew(BEGINNING_TIME_OF_TODAY);
			createTransactionModalCreateBtn.unbind().bind("click",function(){
				var tableId=createTransactionModalTableSelect.val();
				var content=createTransactionModalContentTextarea.val();
 ...
 * 
 * TransactionItem(Transaction_New)
 * 		show()
 * 		hide()
 * 		
 ...
 * 		
 * 		onChange(CALL_BACK(ID,Content,Time))	//当transaction变化时，你可以做一些事情，返回Deferred对象
 * 		onDelete(CALL_BACK(ID))					//当transaction被删除时，你可以做一些事情，返回Deferred对象
 */
var TransactionItem={
	creatNew:function(TRANSACTION_NEW){
		var TransactionItem={};

		var transaction=TRANSACTION_NEW;
		var visible=true;
		var textTranslator=TextTranslator.creatNew();
		var div=Div.creatNew();
		var btn=Button.creatNew();
		...
		(function(){
			...
			if(transaction.isManager()){
				var changeTransactionModal=ChangeTransactionItemModal.creatNew();
				changeTransactionModal.onChange(function(CONTENT,TIME){
					var def=e_change(transaction.getTransactionId(),CONTENT,TIME);
					def.done(function(){
				...
				changeTransactionModal.onDelete(function(){
					var def=e_delete(transaction.getTransactionId());
					def.done(function(){
						changeTransactionModal.hide();
					});
					return def;
				});
				changeTransactionModal.bindModal(btn);
				btn.onClickListener(function(){
					changeTransactionModal.initBeforShow(transaction.getTime(),transaction.getContent(),transaction.getTableName());
				});
			}
			else{
				btn.onClickListener(function(){
		...
		function initTransactionItemUI(){
			...
			btn.html("<span>"+textTranslator.encodeEnterToSpacing(transaction.getContent())+"</span>");
			setPopover(transaction.getTableName(),"<strong>"+time.getHours()+":"+time.getMinutes()+"</strong>"+"<br/>"+textTranslator.encodeText(transaction.getContent()));
		}
 ...
 *   	hide()
 */
var ChangeTransactionItemModal={
	creatNew:function(){
		var ChangeTransactionItemModal={};
		...
		ChangeTransactionItemModal.initBeforeShow=function(TIME,CONTENT,TABLE_NAME){
		...
		ChangeTransactionItemModal.bindModal=function(BUTTON){
			BUTTON.attr("data-toggle","modal");
			BUTTON.attr("data-target","#change_log_transaction_modal");
		}

		ChangeTransactionItemModal.onChange=function(CALL_BACK){
			e_change=CALL_BACK;
		}

		ChangeTransactionItemModal.onDelete=function(CALL_BACK){
			e_delete=CALL_BACK;
		}

		ChangeTransactionItemModal.show=function(){
			changeTransactionModal.modal("show");
		}

		ChangeTransactionItemModal.hide=function(){
			changeTransactionModal.modal("hide");
		}

		return ChangeTransactionItemModal;
	}
}
 ...
 * 		isManager()
 *
 * 		setTableAnotherName(anotherName)
 * 		getTableAnotherName()
 *
...
var AttentionTable={
	creatNew:function(){
		...
		AttentionTable.setTableAnotherName=function(ANOTHER_NAME){
			tableAnotherName=ANOTHER_NAME;
		}

		AttentionTable.getTableAnotherName=function(){
			return tableAnotherName;
		}
 ...
 * 	ID？
 * 
 * Transaction_New(attentionTableAry,transaction)
 * 		isDirectAttention()
 * 		isManager()   		
 * 		getTableName()
 */
var Transaction_New={
	creatNew:function(ATTENTION_TABLE_ARY,TRANSACTION){
		var Transaction_New=TRANSACTION;
		...
		(function(){
			$.each(attentionTableAry,function(index, el) {
				if(el.getTableId() == Transaction_New.getTableId()){
					isDirectAttention=true;
					isManager=el.isManager();
					tableName=el.getTableAnotherName();
				}
			});
			if(!isDirectAttention){
				$.each(attentionTableAry,function(index, el) {
					var result=el.findParentTable(Transaction_New.getTableId());
					if(result != null){
						parentTable=result;
						tableName=el.getTableAnotherName();
					}
		...
		Transaction_New.isDirectAttention=function(){
			return isDirectAttention;
		}

		Transaction_New.isManager=function(){
			return isManager;
		}

		Transaction_New.getTableName=function(){
			if(isDirectAttention){
				return tableName;
			}
			else{
				return parentTable.getTableName()+" >> "+tableName;
			}
		}
